    ...
    while(aux != NULL) {
        for (i = 0; i < M; i++) {
            if(caixa[i].tempo == 0) {
                ...
                if (aux != NULL && caixa[i].total >= aux->Oi) {
                    caixa[i].tempo = aux->Ci;
                    caixa[i].total = caixa[i].total + aux->Ci;
...
//erro ate entao, quando adiciona o tempo que falta pra chegar ele nao toma em consideração os casos em que nao possui proximo e ja deixa somado